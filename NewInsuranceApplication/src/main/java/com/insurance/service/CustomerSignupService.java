...
public class CustomerSignupService 
{
	...
	public ResponseEntity<String> createCustomer(CustomerSignupDetails customer)
	{
       ...
       String email = customer.getEmail();
        ...
        Optional<CustomerSignupDetails> data = customerSignupRepository.findByMobilenoOrEmail(mobileNo,email);
        
        if(data.isEmpty())
        {
        	customerSignupRepository.save(customer);
        	return ResponseEntity.status(HttpStatus.OK).body("Data was inserted successfully");
        }
        else
        {
	    ...
	    public ResponseEntity<String> updateCustomerById(long id, CustomerSignupDetails customer) {
	        ...
	        if (existingCustomerOptional.isPresent()) {
	            ...
	            if (isMobileChanged || isEmailChanged) {
	                ...
	                Optional<CustomerSignupDetails> existingData = customerSignupRepository.findByMobilenoOrEmail(newMobileNo, newEmail);
